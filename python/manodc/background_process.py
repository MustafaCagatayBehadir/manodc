...
class Process(threading.Thread):
    ...
    *no locks* can be "forked" over to the worker while not being unlocked. We
    ...
    # the annotaton right
    mp_ctx: ClassVar[multiprocessing.context.SpawnContext] = cast(multiprocessing.context.SpawnContext,
                                                                  multiprocessing.get_context('spawn'))

    def __init__(self,
                 app,
                 bg_fun: Callable[..., Any],
                 bg_fun_args: Optional[Tuple[Any, ...]] = None,
                 config_path=None,
                 ha_when: HaWhen = HaWhen.PRIMARY,
                 backoff_timer=1,
                 run_during_upgrade=False):
        ...
        self.log = app.log
        self.name = "{}.{}".format(self.app.__class__.__module__, self.app.__class__.__name__)
        ...
        with ncs.maapi.Maapi() as m:
            with ncs.maapi.Session(m, '{}_supervisor'.format(self.name), 'system'):
                ...
                with m.start_read_trans(db=ncs.OPERATIONAL) as oper_t_read:
                    ...
                    if self.ha_enabled:
                        ha_mode = str(
                            ncs.maagic.get_node(oper_t_read, f'/tfnm:ncs-state/{_TFNM_PREFIX}:ha/{_TFNM_PREFIX}:mode'))
            ...
            try:
                ...
                if should_run:
                    ...
                    if not self.config_enabled:
                        ...
                        self.log.info(
                            f"Background worker will not run when HA-when={self.ha_when.value} and HA-mode={self.ha_mode}"
                        )
    ...
    def worker_stop(self):
        ...
        try:
            ...
            if str(exc) == "can only join a started process":
                ...
                raise exc
        if self.worker.is_alive():
            self.log.error("{}: worker not terminated on time, alive: {}  process: {}".format(
                self, self.worker.is_alive(), self.worker))
...
class LogConfigSubscriber(object):
    ...
    def __init__(self, q, vmid):
        ...
        with ncs.maapi.single_read_trans('', 'system') as t_read:
            ...
            try:
                self.vm_level = ncs.maagic.get_node(t_read,
                                                    '/python-vm/logging/vm-levels{{{}}}/level'.format(self.vmid))
            except Exception:
                self.vm_level = None
